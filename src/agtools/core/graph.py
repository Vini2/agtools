...
class AssemblyGraph:
        ...
        with open(path) as f:
            for line in f:
                if line.startswith("S"):
                    ...
                    ag._add_oriented_links(from_seg, to_seg, from_orient, to_orient, overlap)
    ...
    def filter_segments(self, min_length: int) -> "AssemblyGraph":
        keep_segs = {s for s, seq in self.segment_sequences.items() if len(seq) >= min_length}
        ...
        new_ag.segment_names = bidict({i: self.segment_names[v.index] for i, v in enumerate(subgraph.vs)})
        ...
        new_ag.segments_lengths = {s: len(seq) for s, seq in new_ag.segment_sequences.items()}
        return new_ag
...
def extract_links_fastg(segments, fixed_overlap: int):
    links = []
