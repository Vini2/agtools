...
# Load data
# -----------------------------------------------------------
df_ag = pd.read_csv("time_logs/agtools_unitig_graph_flye_profiling_res.csv")
df_gf = pd.read_csv("time_logs/gfapy_unitig_graph_flye_profiling_res.csv")
...
plt.title("Running time - loading Flye GFA")
...
plt.title("Memory usage - loading Flye GFA")
plt.gca().xaxis.set_major_formatter(FuncFormatter(lambda v, _: f"{v:.1f}"))
plt.grid(True)
plt.legend()
