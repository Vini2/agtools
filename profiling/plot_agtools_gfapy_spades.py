...
# Load data
# -----------------------------------------------------------
df_ag = pd.read_csv("time_logs/agtools_unitig_graph_spades_profiling_res.csv")
df_gf = pd.read_csv("time_logs/gfapy_unitig_graph_spades_profiling_res.csv")
...
plt.title("Running time - loading SPAdes GFA")
plt.gca().xaxis.set_major_formatter(FuncFormatter(lambda v, _: f"{v:.1f}"))
plt.grid(True)
plt.legend()
plt.savefig("plots/runtime_spades_agtools_vs_gfapy.png", dpi=300, bbox_inches="tight")
plt.show()
...
plt.title("Memory usage - loading SPAdes GFA")
plt.gca().xaxis.set_major_formatter(FuncFormatter(lambda v, _: f"{v:.1f}"))
plt.grid(True)
plt.legend()
plt.savefig("plots/memory_spades_agtools_vs_gfapy.png", dpi=300, bbox_inches="tight")
plt.show()
