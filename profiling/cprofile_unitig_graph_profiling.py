...
def main():
    ...
    # Save results
    df.to_csv("data/unitig_graph_cprofile_res.csv", index=False)

    # Read the profiling results from CSV file
    df_results = pd.read_csv("data/unitig_graph_cprofile_res.csv")
    ...
    plt.title("Running time vs number of GFA lines for unitig graph")
    plt.grid(True)
