...
def profile_get_contig_graph(graph_file, contigs_file):
    ...
    mem_trace, _ = memory_usage(
        (myloasm.get_contig_graph, (graph_file, contigs_file)),
        retval=True,
        interval=0.01,
        timeout=None
    )
...
def main():
    ...
    plt.title("Running time vs size of the graph file for myloasm contig graph")
    plt.grid(True)
    ...
    plt.title("Peak memory vs size of the graph file for myloasm contig graph")
    plt.grid(True)

    # Save to file
    plt.savefig("plots/myloasm_mem.png", dpi=300, bbox_inches='tight')
    plt.show()
